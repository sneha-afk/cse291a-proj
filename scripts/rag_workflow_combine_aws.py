...
def rewrite_prompt_csv(original_question: str) -> str:
    """
    Rewrite user query for CSV-based retrieval (stock price / tabular data).
    You already add headers to CSVs; this leans into that.
    """
    sys_prompt = """
You are generating a retrieval query for CSV financial data.

The CSV files contain structured stock information with headers like:
- company, ticker, date, date_ts, year, quarter, metric, value, currency
- possibly: segment, statement_type, source, etc.

Rewrite the user's question into a short, keyword-heavy query that:
- Mentions specific tickers, companies, years, and quarters if implied.
- Uses likely header names (ticker, date_ts, year, quarter, metric, segment).
- Is optimized to match CSV row content and headers, NOT prose.
